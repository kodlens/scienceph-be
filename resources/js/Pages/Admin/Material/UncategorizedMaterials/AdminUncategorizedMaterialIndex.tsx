...
const AdminUncategorizedMaterialIndex = ( {auth}:PageProps ) => {
  ...
  const { data, isFetching, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await axios.get(`/admin/get-uncategorized-materials?${params}`);
      return res.data;
    },
  });
  ...
  return (
    <>
      <Head title="Uncategorized Materials" />
...
AdminUncategorizedMaterialIndex.layout = (page: ReactElement) => <AuthenticatedLayout user={page.props.auth.user}>{page}</AuthenticatedLayout>
export default AdminUncategorizedMaterialIndex;
